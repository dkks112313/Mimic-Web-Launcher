        ...
        if native != "":
            if "downloads" in i and "path" in i["downloads"]["classifiers"][native]:  # type: ignore
                libstr += os.path.join(path, "libraries", i["downloads"]["classifiers"][native][
                    "path"]) + classpath_seperator  # type: ignore
            else:
...
def replace_arguments(argstr: str, versionData: ClientJson, path: str, options: MinecraftOptions,
                      classpath: str) -> str:
    """
...
def get_arguments(data: List[Union[str, ClientJsonArgumentRule]], versionData: ClientJson, path: str,
                  options: MinecraftOptions, classpath: str) -> List[str]:
    """
...
def get_minecraft_command(version: str, minecraft_directory: Union[str, os.PathLike], options: MinecraftOptions) -> \
List[str]:
    """
